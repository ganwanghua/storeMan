...
public class RemotDataSourceImpl implements RemotDataSource {
    ...
    @Override
    public void updateImei(Map<String, String> queryMap, final getCallback callback) {
        Observable<StatusBean> observable = RetrofitHelper.getInstance(mContext).getServer().updateImei(queryMap);
        observable.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<StatusBean>() {
                    @Override
                    public void onCompleted() { // 完成请求后
    ...
    @Override
    public void withdraw(Map<String, String> queryMap, final getCallback callback) {
        Observable<StatusBean> observable = RetrofitHelper.getInstance(mContext).getServer().withdraw(queryMap);
        observable.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<StatusBean>() {
                    @Override
                    public void onCompleted() { // 完成请求后
